...
def calc_gradients(model_name, image_producer, output_file_dir=None, max_iter = 1, noise_file = None, learning_rate = 0.001 * 255, use_round=False, weight_loss2 = 1, data_spec=None, batch_size=1):
    """Compute the gradients for the given network and images."""
    
    spec = models.get_data_spec(model_name)

    modifier = tf.Variable(np.zeros((batch_size, spec.crop_size, spec.crop_size, spec.channels), dtype=np.float32))
    input_image = tf.placeholder(tf.float32, (None, spec.crop_size, spec.crop_size, spec.channels))
    input_label = tf.placeholder(tf.int32, (None))

    true_image = tf.minimum(tf.maximum(modifier + input_image, -spec.mean + spec.rescale[0]), -spec.mean + spec.rescale[1])
    ...
    # In paper it's mentioned set lambda to 0, i.e. not restricting d(x, x*) to let Adam optimizer explore the surrounding space
    ...
    # Adding noise
    noise = np.zeros(shape=(total, spec.crop_size, spec.crop_size, spec.channels), dtype=float)
    if noise_file is not None:
        noise = convert_noise(np.load(noise_file), spec.crop_size, spec.crop_size)

    gradient_record = np.zeros(shape=(total, spec.crop_size, spec.crop_size, spec.channels), dtype=float)
    rec_iters = []
    rec_names = []
    rec_dist = []
    ...
    with tf.Session() as sesh:
        ...
        varibale_list = (set(tf.global_variables()) - temp_set).union(set([modifier]))
        sesh.run(tf.variables_initializer(varibale_list))

        # Load the parameters to model
        print 'Loading parameters...', model_name
        if model_name == "Inception":
            models.inception_load_parameters(sesh, set(tf.all_variables()) - varibale_list)
        else:
        ...
        for (indices, labels, names, images) in image_producer.batches(sesh):
            ...
            var_loss, true_prob, var_loss1, var_loss2 = sesh.run((loss, true_label_prob, loss1, loss2), feed_dict=feed_dict)
            ...
            for cur_iter in range(max_iter):
                ...
                if cur_iter + 1 == max_iter or break_condition:
                    var_diff, var_probs = sesh.run((diff, probs), feed_dict=feed_dict)
            ...
            if use_round:
                noise_diff = np.sign(noise_diff) * np.ceil(np.absolute(noise_diff))
            for i in range(len(indices)):
...
def main():
    ...
    parser.add_argument('-o', '--output_dir', type=str, default=None, help='Directory of output noise file.')
    parser.add_argument('--model', type=str, required=True,
                        choices=['Inception', 'ResNet50', 'ResNet101', 'ResNet152', 'VGG16', 'AlexNet', 'GoogleNet'],
                        help='Models to be evaluated.')
    parser.add_argument('--num_images', type=int, default=sys.maxint, help='Max number of images to be evaluated.')
    parser.add_argument('--file_list', type=str, default=None,
                        help='Evaluate a specific list of file in dataset.')
    parser.add_argument('--num_iter', type=int, default=1000, help='Number of iterations to generate attack.')
    parser.add_argument('--learning_rate', type=float, default=0.001 * 255, help='Learning rate of each iteration.')
    parser.add_argument('--use_round', dest='use_round', action='store_true', help='Round to integer.')
    parser.add_argument('--weight_loss2', type=float, default=1, help='Weight of distance penalty.')
    parser.add_argument('--noise_file', type=str, default=None, help='Directory of added noise.')
    ...
    image_producer = dataset.ImageNetProducer(file_list=args.file_list, data_path=args.input_dir,
                                              num_images=args.num_images, data_spec=data_spec, batch_size=1)

    calc_gradients(args.model, image_producer, args.output_dir, args.num_iter, args.noise_file, args.learning_rate, args.use_round, args.weight_loss2, data_spec, batch_size=1)
