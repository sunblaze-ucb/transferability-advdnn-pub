...
def calc_gradients(sesh, image_producer, input_node, probs, data_spec):
    ...
    gradient_record = np.zeros(shape=(total, data_spec.crop_size, data_spec.crop_size, data_spec.channels), dtype=float)
    ...
    for (indices, label, name, images) in image_producer.batches(sesh):
        ...
        for i in range(len(indices)):
            new_images[i] = np.clip(new_images[i] + data_spec.mean, data_spec.rescale[0], data_spec.rescale[1])

        gradients = sesh.run(var_grad, feed_dict={input_node:new_images, grad_ys_node: one_hot})
        for i in range(len(indices)):
            gradients[0][i] /= np.sqrt(np.reduce_mean(np.square(gradients[0][i].flatten())))
...
def save_file(sesh, image_producer, tmp_dir, noise):
    ...
    for (indices, label, name, images) in image_producer.batches(sesh):
        for index in range(len(indices)):
            attack_img = np.clip(images[index] + noise[indices[index]] + data_spec.mean, data_spec.rescale[0], data_spec.rescale[1])
...
def evaluate(sesh, image_producer, input_node, probs, data_spec):
    ...
    for (indices, labels, names, images) in image_producer.batches(sesh):
        ...
        print('{:>6}/{:<6} {:>6.2f}% {:>6.2f}%'.format(count, total, cur_top1_accuracy, cur_top5_accuracy))
...
def main():
    ...
    parser.add_argument('-t', '--tmp_dir', type=str, required=True, help='Directory of dataset.')
    ...
    parser.add_argument('--model2', type=str, required=True,
                        choices=['Inception', 'ResNet50', 'ResNet101', 'ResNet152', 'VGG16', 'AlexNet', 'GoogleNet'],
                        help='Models to be evaluated.')
    parser.add_argument('--file_list', type=str, required=True, help='Evaluate a specific list of file in dataset.')
    ...
    image_producer_o = dataset.ImageNetProducer(file_list=args.file_list, data_path=args.input_dir, num_images=args.num_images, need_rescale=True, data_spec=data_spec1)
    image_producer1 = dataset.ImageNetProducer(file_list=args.file_list, data_path=args.tmp_dir, num_images=args.num_images, need_rescale=False, data_spec=data_spec1)

    data_spec2 = models.get_data_spec(model_name=args.model2)
    image_producer2 = dataset.ImageNetProducer(file_list=args.file_list, data_path=args.tmp_dir, num_images=args.num_images, need_rescale=False, data_spec=data_spec2)
    ...
    gradients = calc_gradients(sesh, image_producer_o, input_node1, probs_output1, data_spec1)

    true_label = image_producer1.get_truth_labels(file_list)
    total = len(image_producer1)
    upper_bound = 10
    while True do:
        save_file(sesh, image_producer_o, args.tmp_dir, gradients * upper_bound)
        predict1 = evaluate(sesh, image_producer1, input_node1, probs_output1, data_spec1)
        predict2 = evaluate(sesh, image_producer2, input_node2, probs_output2, data_spec2)
        ok = True
