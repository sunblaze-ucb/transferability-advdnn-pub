...
def calc_gradients(model_name, image_producer, args):
    ...
    for (indices, labels, names, images) in image_producer.batches(sesh):
        ...
        original_dir = sesh.run(
            var_grad,
            feed_dict={
                input_node: images,
                input_label: labels})[0][0]
        ...
        for iterx in range(args.num_tries):
            ...
            for i in range(new_images.shape[0]):
                noise = np.random.randn(
                    spec.crop_size, spec.crop_size, spec.channels)
                noise -= dot(noise, original_dir) * original_dir
                noise = noise / np.sqrt(np.mean(np.square(noise))) * \
                    args.std_dev / 255.0 * (spec.rescale[1] - spec.rescale[0])
...
def main():
    ...
    parser.add_argument('--num_tries', type=int, default=5,
                        help='Number of time to add noise.')
    parser.add_argument('--std_dev', type=float, default=1,
                        help='Std dev of noise, out of 255')
